...
my_target = 'AD'
my_target = 'VaD'
output_img_dir = dpath + 'Results/'+ my_target + '/MetPanel/s3_SFS_Plot.png'
met_code = pd.read_csv(dpath + 'Data/MetabolomicData/MetCode.csv', usecols = ['Met_code', 'Met_name'])
met_imp_df = pd.read_csv(dpath + 'Results/'+ my_target + '/MetPanel/s0_MetImportance.csv')
met_imp_df.rename(columns = {'TotalGain_cv': 'Met_imp'}, inplace = True)
met_auc_df = pd.read_csv(dpath + 'Results/'+ my_target + '/MetPanel/s1_AccumAUC.csv')
mydf = pd.merge(met_auc_df, met_imp_df, how = 'left', on = ['Met_code'])
mydf = pd.merge(mydf, met_code, how = 'left', on = ['Met_code'])
...
palette.reverse()
sns.barplot(ax=ax, x = "Met_name", y = "Met_imp", palette=palette, data=mydf.sort_values(by="Met_imp", ascending=False))
y_imp_up_lim = round(mydf['Met_imp'].max() + 0.01, 2)
ax.set_ylim([0, y_imp_up_lim])
ax.tick_params(axis='y', labelsize=14)
ax.set_xticklabels(mydf['Met_name'], rotation=45, fontsize=10, horizontalalignment='right')
my_col = ['r']*nb_f + ['k']*(len(mydf)-nb_f)
for ticklabel, tickcolor in zip(plt.gca().get_xticklabels(), my_col):
    ticklabel.set_color(tickcolor)

ax.set_ylabel('Metabolomic Predictor Importance', weight='bold', fontsize=18)
